...
{
    bool TwoViewReconstruction::ReconstructEmt(const std::vector<cv::KeyPoint> &vKeys1,
                                            ...
                                            Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
    {
        ...
        // Indices for minimum set selection（最小集选择的索引）
        ...
        // 从mvMatches12中随机挑选8组匹配为一个set，一共mMaxIterations（默认200）个set
        ...
        // 三角化重投影成功匹配点的视差角（单位）的最小值
//        float minParallax = 1.0;
        float minParallax = 0.5;
